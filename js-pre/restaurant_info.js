...
fillReviewsHTML = (reviews) => {
  ...
  var f = document.createElement("form");
        ...
        i2.name = "name";
        i2.id = "name";
        i2.style="margin:5px;text-align:center;width:150px;height:20px;border-radius:8px;"
        ...
        i3.name = "rating";
        i3.id = "rating";
        i3.style="margin:5px;text-align:center;width:150px;height:20px;border-radius:8px;"
        ...
        i4.cols=50;
        i4.rows=5;
        i4.style="padding:10px;margin:10px;width:300px;border-radius:8px;"
...
function offlinePosting(){
  var x=localStorage.getItem('gotData');
  if(x=='true'){
  //localStorage.setItem('gotData',false);
  var data={restaurant_id:Number(localStorage.getItem('restaurant_id')),name:localStorage.getItem('name'),rating:Number(localStorage.getItem('rating')),comments:localStorage.getItem('comments')}
  localStorage.clear();
...
.then((res) => {
  ...
  return updateReviewsIdb()
})
.then(() => {
const ul = document.getElementById('reviews-list');
ul.appendChild(updateReviewHTML(data))
const container = document.getElementById('reviews-container');
container.appendChild(ul);
});
}
...
function postReview(){
  var rest_id, rName, rRating, rComments;
  rest_id=document.getElementById('restaurant_id').value;
  rName=document.getElementById('name').value;
  rRating=document.getElementById('rating').value;
  rComments=document.getElementById('comments').value;
  var data = {restaurant_id:Number(rest_id),name:rName,rating:Number(rRating),comments:rComments};  
  if(navigator.onLine==true){
    ...
    localStorage.setItem('restaurant_id',Number(rest_id));  
    localStorage.setItem('name',rName);  
    localStorage.setItem('rating',Number(rRating));  
    localStorage.setItem('comments',rComments); 
    data.createdAt=new Date().toJSON();
    data.updatedAt=new Date().toJSON();
    document.getElementById('btn').style.backgroundColor="red";
    document.getElementById('btn').style.color="white";
    document.getElementById('btn').value="Offline";
    document.getElementById('btn').disabled=true;
    const ul = document.getElementById('reviews-list');
    ul.appendChild(updateReviewHTML(data))
    const container = document.getElementById('reviews-container');
...
function updateReviewsIdb(datta){
  return idb.open('reviews', 1)
      .then(function(db){
        ...
        console.log(datta);
          ...
          console.log('Reviews - all items updated successfully!');
        }) 
        .catch(function(e) {
          tx.abort();
          console.log(e);
